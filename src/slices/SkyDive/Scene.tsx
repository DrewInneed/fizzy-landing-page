...
export default function Scene({ sentence, flavor }: SkyDiveProps) {
  ...
  useGSAP(() => {
    ...
    scrollTl
      ...
      // move the clouds back to the screen (which are already running up infinite)
      .to(cloudsRef.current.position, { z: 0, duration: 0.3 }, 0)
      // move the can back to the scene from top down
      ...
      // move each of the children text up from below
      .to(
        wordsRef.current.children.map((word) => word.position),
        {
          keyframes: [
      ...
      // shoot the clouds out onto the screen
  ...
  return (
    <group ref={groupRef}>
      ...
      <Clouds ref={cloudsRef}>
        <Cloud ref={cloud1Ref} bounds={[10, 10, 5]} />
        <Cloud ref={cloud2Ref} bounds={[10, 10, 5]} />
      </Clouds>

      {/* Text */}
      <group ref={wordsRef}>
        {sentence && <ThreeText sentence={sentence} color="#e0041e" />}
      </group>
